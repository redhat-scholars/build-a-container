...
. Next let's poke around the directory where visitors were getting logged
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
----
+
. And finally, this means that any secrets on the host are exposed or manipulatable by the intruder
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
