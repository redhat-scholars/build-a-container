...
* xref:SECURITY:containers-and-security.adoc#privileged_containers[don't run privileged containers,window=_blank]
* xref:SECURITY:containers-and-security.adoc#se_linux[use SELinux to fine tune container access to volume mounts,window=_blank]
* don't run containers as root
* ensure that your containers do not include known vulnerabilities or CVEs

For the purposes of this lab, we're only going to investigate the last two items on this list.  (_NOTE: If you'd like to go deeper on the first two points, follow the links provided_)
...
----
+
. For most of the lab we have been running as the `root` user.  Double check that you're still running as root by running
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
Let's return to our metasploit terminal for the next section
...
+
--
. Reopen the exploit in the other terminal by entering following the instructions link:containers-and-security.html?USER={USER}&CLUSTER_SUBDOMAIN={CLUSTER_SUBDOMAIN}#exploit_shellshock[above^] to create a bash terminal on the container 
