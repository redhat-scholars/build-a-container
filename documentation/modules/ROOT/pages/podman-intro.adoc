...
[WARNING]
====
Some of you with a security background are [#security_vuln]#probably fairly unconfortable with this#.  Don't worry: there's a method to our madness!  It's worthLook inside a running container pointing out that any socket-based container runtimes (such as `docker`) require some sort of root access to operate
====
+
{zwsp} +
...
----
<.> This flag, as we'll come to see, could be problematic.  It allows the container to run with an extended set of capabilities that might otherwise be reserved for the host (such as accessing ports < 1024).  However, when running some processes in a container, this is sometimes required
...
With `podman` the command is `commit` footnote:[For more info on committing container state, see the link:http://docs.podman.io/en/latest/markdown/podman-commit.1.html[podman documentation]
].  Let's issue the command now and we'll see some of the value in it <<rerunning_container,a little later on in the lab>>.
...
+
{zwsp} +
. How about our `visitor_info.txt` file?  Let's copy that out of the container again
+
include::partial$copy_visitor_info.adoc[]
+
{zwsp} +
. But if you open the `visitor_info.txt` after copying it out, you'll notice that all the visitor names are gone!
