...
+
{zwsp} +
. Next, let's use the VS Code editor to see what our command found out. Use the following to load the file in the editor.
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
+
{zwsp} +
. You should now see the file appear in your *Explorer*.  Click on it to view the contents.  It should match the visitors that you entered previously. Alternative run the following command to load it.
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
