...
. Login as `root` on your instance by running the following command:
+
[.console-input]
...
+
image::apache-guestbook-submit.png[]
+
. Click the `here` link highlighted above to go back and enter another name
{zwsp}+
...
+
{zwsp} +
. To make it easier to read, let's copy the hostinfo to the `/var/www/html` directory
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
----
+
_NOTE: The `/var/www/html` directory is the web root of the apache server that is running inside this container] so that we can view it through the container's webserver._
+
{zwsp} +
...
----
+
_NOTE: We're not really logging out as much as we're causing the process that we `podman exec` ed to stop thus ending the `podman exec` command and returning us to the host's terminal_
+
{zwsp}
...
----
podman cp \
  {apache-server-container-name}:/tmp/hostinfo.txt \#<.>
...
+
{zwsp} +
. With the `{temp-file}` still in focus, hit kbd:[SHIFT+CTRL+p] (or kbd:[CMD+CTRL+p] on a Mac) to open the *Command Palette* and start typing `compare` so that you can select the command `File: Compare Active File With...`
+
...
+
image::visitor-info.png[Visitor Info]
...
+
--
. Luckily, we have already gotten the container's ID and stored it in `CONTAINER_ID` (see <<find_container_id,here>> if you skipped that part).  
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
With `podman` the command is `commit`

_NOTE:For more info on committing container state, see the link:http://docs.podman.io/en/latest/markdown/podman-commit.1.html[podman documentation,window=blank]_

Let's issue the command now; we'll see some of the value in it <<rerunning_container,a little later on in the lab>>.
...
[TIP]
====
As with the other `podman` commands, it's possible to remove containers by **CONTAINER ID**.  In that case you'd just issue this command (assuming you followed directions above to store the container id in `pass:[${CONTAINER_ID}]`)
...
+
{zwsp} +
. How about our `visitor_info.txt` file?  Let's delete the current one by right-clicking on it in the Explorer and selecting `Delete Permanently`
. Now attempt to copy visitor_info.txt out of the container again
+
include::partial$copy_visitor_info.adoc[]
+
{zwsp} +
...
=== 8. "Re-running" a stopped container by image

Before we through our hands up in disgust, remember that we had the foresight of _committing_ our previously running web server container to a new image called `{apache-committed-container-image}`.  This image should container all the changes we made on top of the `{apache-server-container-name}` that we just ran and allow us access to the info that does not exist in the `{apache-server-image-insecure}`.

{zwsp}

. First let's stop and remove the current `{apache-server-image-insecure}` image based web server
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
+
.Container's browser once again displays our {temp-file}
image::browser-display.png[]
+
{zwsp} +
. And let's restore our `{guestbook-record-file}` by pulling it out of the container running based on the committed image by issuing the same command as before
+
include::partial$copy_visitor_info.adoc[]
+
{zwsp} +
...
[#automatic_container]
== 9. Start Containers automatically
...
This is handy for containers which you want to run, and ensure that they keep running.
...
We won't set this up in this lab, but the following steps would generate and install the service, enable and start the service so that everytime you reboot
