...
. First, let's record what's going on with the container host.  Run the following command in the terminal to gather some information about the virtual machine Code Server is running on
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
----
+
. Now let's get into the container so that we can run the same command.  We can "enter" the container by way of the `podman exec` command: 
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
<.> ${CONTAINER_ID} should be holding the id of the running container that we saw with `podman ps`
...
+
. If the command is successful, you should see the following in the terminal
+
[.console-output]
[source,bash,subs="+macros,+attributes"]
----
...
----
+
. Now let's run our command from before but this time within the container
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
. Within the container, run the following command to show that the output is different
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
----
+
. To make it easier to read, since we appear to be `root` inside the container, let's copy the hostinfo to the `/var/www/html` directory so that we can view it through the container's webserver
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
+
. Now click on the left editor where the `{temp-file}` should still be open.  Hit kbd:[SHIFT+CTRL+p] (or kbd:[CMD+CTRL+p] on a Mac) to open the *Command Palette* and start typing `compare` so that you can select the command `File: Compare Active File With...`
+
image::compare-command.png[]
+
. Then select the {container-info-filepath} in the next dialog that pops up
+
...
But what happened to our file or, put another way, the changes we made to our container?  Let's look to the next section to find out.