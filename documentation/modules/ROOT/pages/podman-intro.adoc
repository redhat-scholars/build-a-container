...
----
+
. Now let's get into the container so that we can run the same command.  But first `podman` needs to know which container we mean.  Once again, let's use the `podman ps` command to get that info and save it into the variable `CONTAINER_ID`
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
. You can stop a container by its ID or by its name.  Luckily, we have already gotten the container's ID and stored it in `CONTAINER_ID` (see <<Look inside a running container,here>> if you skipped that.  Issue the following `podman` command from the terminal to stop the container:
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
