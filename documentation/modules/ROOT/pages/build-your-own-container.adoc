...
----
+
. Once the container is running, as before, paste the following in the Browser Preview navbar: 
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
----
+
. And you should now see the guestbook:
+
.Guestbook restored
image::apache-guestbook-page.png[]
+
. Go ahead and "sign" the guestbook by entering your *name* (such as "Alice") and pressing `Log to Guestbook`.  When you do so, you should be greeted with a page like this:
+
...
. Let's edit the copy of the httpd.conf we pulled out of our original container earlier.  Click on the httpd.conf to edit it in VSCode
+
