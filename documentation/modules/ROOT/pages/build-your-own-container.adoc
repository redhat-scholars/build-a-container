...
`buildah` is used to create OCI-compliant container images, such as the `{apache-server-image-insecure-image}` that we've been using.  We'll start by fixing a the security vulnerability we found in the xref::containers-and-security.adoc[previous section,window=_blank].
...
. First make sure there are no containers running with the `{apache-server-container-name}`
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
[TIP]
====
If there was a container running you will see something like
...
And if there were none running you'll see something like
...
Getting either output it's fine to proceed
====
+
